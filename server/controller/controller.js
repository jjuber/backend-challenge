...
const getTags = (req, res) => {
  ...
  // Gives an error if the sortBy/direction value are not valid.
  ...
  // When the user requests more than one tag, we will precede with the following
  if (tags.indexOf(',') !== - 1) {
    ...
    // There are nine possible tags that can be inputted, so they all need to be considered
    // If the value is not there, this will be ignored using an empty string
    // Null/undefined are not used because I will use the length method below on the data array
      .then(axios.spread((tag1, tag2, tag3, tag4, tag5, tag6, tag7, tag8, tag9) => {
        ...
        // This is the most efficient way to get rid of duplicates is to make a hash table
        ...
        // Finally we remove duplicates and sort our data as the user desires and can return the data
        res.status(200).send(posts);
      }))
      // When no tags are used, then this error will be shown to the user
      .catch(error => {
        res.status(400).send({
    ...
    // If the user only searches for one tag, then we don't have to worry about duplicate values.
